...
function translate(sentence) {
   return sentence.split(' ').map(word => {
    let string=(word.match(/[?!]/gi))?word.substring(0,word.search(/[?!]/gi)):word

    let toVowel=string.substring(0,string.search(/[aeiou]/i));
    let remains=string.slice(toVowel.length)
    console.log(string)
    let rest=(string.charAt(0).match(/[A-Z]/g))?remains.charAt(0).toUpperCase()+remains.substring(toVowel.length):remains.substring(toVowel.length);
    if(!string.match(/[aeiou]/gi)){
        return (string+"ay");
    }else if(toVowel.length===0){
        return (string+"way");
    } else{
    return( rest+toVowel+"ay");
    }
